...
class BouncingCircle(gctx: GameContext) : IGameObject {
    ...
    val x = Random.nextFloat() * (gctx.metrics.width - 2 * radius) + radius // 원이 화면 밖으로 나가지 않도록 위치 설정
    var y = Random.nextFloat() * (gctx.metrics.height - 2 * radius) + radius // 원이 화면 밖으로 나가지 않도록 위치 설정
    ...
    override fun update(gctx: GameContext) {
        ...
        if (speed > 0 && y >= gctx.metrics.height - radius) {
            speed = -speed
            if (abs(speed) < 20f) {
